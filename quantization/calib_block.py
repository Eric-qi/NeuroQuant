...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, cali_data: torch.Tensor, 
    ...
    Args:
            model: QuantModel
            block: the block to be calibrated
    ...
    cached_inps, cached_outs = save_inp_oup_data(model, block, cali_data, asym, batch_size=10, input_prob=True, keep_gpu=False)
    cached_time = time.time() - cached_start
    logging.info('Cached init time: {}'.format(cached_time))
    
    if opt_mode != 'mse':
        cached_grads = save_grad_data(model, block, cali_data, batch_size=1)
    else:
        cached_grads = None
    ...
    for name, module in block.named_modules():
        if isinstance(module, QuantModule):
            ...
            module.bias_quantizer.soft_targets = False
