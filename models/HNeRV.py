...
from ._layers import ConvNeXt, OutImg, NeRVBlock
...
class HNeRV(nn.Module):
    def __init__(self, cfg):
        ...
        assert cfg['enc_strides'] == cfg['dec_strides']
        
        # BUILD Encoder LAYERS
        crop_h, crop_w = cfg['crop_h'], cfg['crop_w'],
        h, w = [crop_h, crop_w] // np.prod(cfg['enc_strides'])
        self.fc_h = np.prod(cfg['enc_strides']) // np.prod(cfg['dec_strides'])
        self.fc_w = np.prod(cfg['enc_strides']) // np.prod(cfg['dec_strides'])
        
        self.encoder = ConvNeXt(stage_blocks=cfg['stage_block'], 
                                strds=cfg['enc_strides'], 
                                dims=cfg['enc_channel'],
                                drop_path_rate=0)
        
        # BUILD Decoder LAYERS
        dec_layers = []
        in_channel = cfg['dec_in_channel']
        dec_layer1 = nn.Conv2d(cfg['enc_channel'][-1], in_channel, 1, 1, 0)
